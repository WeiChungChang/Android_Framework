...
public:
    ...
    ~weakref_impl()
    {
        ...
        if (!mRetain && mStrongRefs != NULL) {
            ...
            while (refs) {
                char inc = refs->ref >= 0 ? '+' : '-';
                printf("\t%c ID %p (ref %d):", inc, refs->id, refs->ref);
#if 0
...

        {
            ...
            if (rc >= 0) {
				...
				//ALOGE("FAILED TO PRINT STACK TRACE for %p in %s: %s", this,
                      //name, strerror(errno));
				printf("STACK TRACE for %p saved in %s", this, name, strerror(errno));
          	}
        }
    }
    ...
    void removeRef(ref_entry** refs, const void* id)
    {
        if (mTrackEnabled) {
...
#endif
#endif
			printf("RefBase: removing id %p on RefBase %p"
                    "(weakref_type %p) that doesn't exist!",
                    id, mBase, this);
#if 0
            ...
            while (ref) {
				...
				printf("\t%c ID %p (ref %d):", inc, ref->id, ref->ref);
                ref = ref->next;
            }
